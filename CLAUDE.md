...
## Mission
An autonomous LangGraph agent that solves live CTF challenges end-to-end — recon, exploit,
flag extraction — grounded in a team knowledge vault and 14 vetted technique packs, for a small
team that can't all afford Claude Pro; the demo-day wow moment is running it through the
dashboard against a real, unseen HackTheBox target with human-in-the-loop approve/deny on every
live network call, and watching the actual flag come back on stage, not a canned sample.
*(Drafted from what's actually built/demonstrated — edit freely if the team wants different
emphasis for the pitch.)*
...
### Session update (2026-08-03/04) — dashboard wired to the real agent, two model bugs found, Google auth-key migration, Cosmic Explorer still unsolved
- **Dashboard was never actually calling the agent.** `dashboard/app/api/agent/route.ts` was still Hasif's stub (`streamText` with a fake model string, hardcoded `flag{stub_placeholder}`) — hitting it directly threw `TypeError: result.toDataStreamResponse is not a function` (removed in the installed `ai` v7). Rewrote it to proxy to `agent/api.py`'s `/solve` endpoint via `createUIMessageStream`/`createUIMessageStreamResponse` (`ai` v7's actual streaming API), reading the backend URL from `AGENT_API_URL` (defaults to `http://localhost:8000`).
- **Second, separate frontend bug**: `dashboard/app/page.tsx`'s `useChat({ api: "/api/agent" })` — that `api` field doesn't exist on `useChat` in this SDK version (only on `useObject`); it was silently ignored, so every real button click fell through to the SDK's hardcoded default endpoint, `/api/chat` (404, nothing there). Fixed with `transport: new DefaultChatTransport({ api: "/api/agent" })`, plus an `onError` handler so a future failure resets `isRunning` instead of leaving the button permanently stuck on "Running...".
- **Two real bugs found in how the model calls `fetch_url`** (not the tool itself — verified `fetch_url` passes `headers` straight to `requests.request()` unmodified): `gemini-3.5-flash-lite` was substituting underscores for hyphens in header names (`X_Forwarded_For` instead of `X-Forwarded-For` — a header the server will never recognize as the real thing), and separately splitting `Content-Type: application/json` at the wrong point (key `Content`, value `-Type: application/json`). Fixed by making the docstring explicitly warn against both — confirmed via a live re-run that the model then sent correctly-hyphenated headers.
- **Google API key migration (`AQ.` "auth keys") — real, unresolved-by-us root cause found, then fixed by generating a new key.** Google AI Studio now issues service-account-bound "auth keys" (`AQ.` prefix, ~50-215 chars) by default instead of legacy `AIzaSy...` keys (phased out entirely by Sept 2026 per Google's own docs). The first key we tried consistently 401'd (`ACCESS_TOKEN_TYPE_UNSUPPORTED` / `API_KEY_SERVICE_BLOCKED`) across every auth method tried — SDK, raw REST with `x-goog-api-key`, raw REST with `Authorization: Bearer`, and `google-genai`'s new `enterprise=True` mode with `project`/`location` (upgraded `google-genai` 2.14.0→2.16.0 for this, didn't help) — ruling out SDK-version, API-enablement, and key-restriction causes one at a time. Root cause never fully confirmed (looked headed toward needing a real OAuth token-exchange step, not a raw string credential) — but **generating a fresh key from the same "auth key" flow worked immediately**, so this may have been a propagation glitch specific to that one key rather than something structurally broken about auth keys in general. Worth retesting if it recurs.
- **`GOOGLE_API_KEYS` (multi-key rotation) now actually in use**: combined the team's working keys (`.env`, comma-separated) — confirmed `_RotatingChatModel` correctly pools across them. Note the precedence: `GOOGLE_API_KEYS`, if set, **replaces** `GOOGLE_API_KEY` entirely rather than adding to it (see `_load_keys()` in `agent/model_router.py`) — a solo key must be included in the list too if you want it as part of the rotation, not left on the singular var.
- **Cosmic Explorer (web CTF challenge, HTB-style, IP rotates on instance expiry)** — exhaustively investigated across 5 separate agent runs (~90 steps total), zero flag, zero positive signal on any front. Fully mapped: `POST /execute` takes `{"action": "getcosmic"}` (200, real data) or `{"action": "getSecureCode"}` (200, but **always** `Access denied: Invalid security clearance`, byte-identical every time). Five hypotheses tried and closed out, all negative: (1) header/cookie/IP-spoofing (`X-Forwarded-For`, `X-Real-IP`, `Referer`, `Cookie`, `Authorization` — all tried, even confirmed via real Chrome DevTools that a totally standard browser request gets denied identically, so this isn't spoofable-header-shaped at all); (2) type confusion on `action` (array/object/number/null/case variants — clean, distinct `400 Invalid JSON` / `400 Invalid command` errors, no bypass); (3) hidden GET routes (`/source`, `/.env`, `/debug`, `/admin`, etc. — server has a catch-all serving the same SPA page for anything except `/execute`, confirmed no hidden route exists); (4) extra JSON body fields alongside `action` (`token`, `apiKey`, `role`, `session`, etc. — no effect); (5) hidden action command names beyond the two UI-exposed ones (18 candidates incl. `getFlag`, `sudo`, `admin`, `bypass` — all uniformly `400 Invalid command`). Also confirmed via direct in-page `fetch()` timing test: `getcosmic` and `getSecureCode` take statistically identical time (~180ms), ruling out a timing side-channel too. **Genuinely stuck** — the real vulnerability is something not yet conceived, not a variation on anything tried so far. Picking this back up needs a fresh angle, not another payload-guessing pass.
- **Workflow note**: this session validated a "human-as-recon, agent-as-solver" workflow — Claude did NOT solve challenges directly; it ran the actual LangGraph agent via `agent/api.py`'s `/solve` endpoint (same backend the dashboard's "Run Agent" button hits) and coached it with better prompts between runs based on reading its own tool-call traces. Chrome-based recon (network tab, timing, headers) was used only to gather facts to feed back into agent prompts, never to construct or supply an exploit payload directly. Worth keeping in mind for organizer scoring concerns raised earlier — no explicit ruling from the organizer exists on this either way.

### Session update (2026-08-04) — venv setup, HITL wired into the dashboard, Space Explorer re-run via the dashboard (honest framing below)
- **`uvicorn` not found → real venv setup, not just a PATH fix.** `pip show uvicorn` found the package, but the console script lived under `%APPDATA%\Python\Python314\Scripts\`, not on `PATH`. Fixed properly: created `.venv/` (added to `.gitignore`) and installed `requirements.txt` into it — also caught that `requirements.txt` was missing `pillow` (needed by `agent/tools/extract_metadata.py`; `agent.api` failed to import without it), a real gap for anyone doing a fresh install, now fixed.
- **`require_approval` (Enforce Permissions) wired all the way through the dashboard**, not just the CLI/API. `dashboard/app/page.tsx` has a "Require approval before live network calls (HITL)" checkbox; `dashboard/app/api/agent/route.ts` forwards it to `/solve` and, when the backend returns `pending_approval`, writes a `data-approval` UI message part (thread ID + the pending tool/args/target) instead of plain text. The page renders that as Approve/Deny buttons; clicking one sends a literal `"approve"`/`"deny"` chat message, and the route recognizes it (by scanning the most recent assistant turn's parts for the `data-approval` marker) and calls `/solve/resume` instead of starting a new run. Verified live, twice, end-to-end against a real external target (see below): one approve → `fetch_url` fired and returned the real response; one deny → `fetch_url` never fired, model reported it was denied by the operator.
- **Space Explorer, re-run through the actual dashboard against a fresh instance — flag captured again (`HTB{REDACTED}`), this time by the agent, not a human.** HITL was on; both `fetch_url` calls (plain `getcosmic`, then the payload with duplicate/differently-cased `action` keys) were approved manually before firing. **Honest framing, corrected after the fact**: the original plan was to test whether a *new, deliberately generic* vault note (`vault/Web_JSON_Key_Casing_Differential.md`, written without naming this challenge) would let the agent generalize the technique to a fresh instance on its own. But a fuller check of the vault (which had grown substantially — 150+ notes under `vault/CTF_Vault/` and `vault/techniques/`, evidently from Farhan's Obsidian sync/the "CTF Brain" work — missed in the original pass, which only spot-checked two files) turned up a **pre-existing, far more specific note already covering this exact case**: `vault/techniques/web/json-parser-key-casing-differential.md`, which names the challenge, states the flag, and cites a cross-checked public writeup. The agent's `search_vault("action")` call surfaced both notes together, so this run mainly demonstrates **retrieval of already-documented prior knowledge working correctly through the full stack** (vault-before-skills grounding priority, live-target tool use, HITL gating) — not the originally-hoped-for "closed a novel reasoning gap" result. Still a genuinely good, real validation of the memory/harness architecture, just a narrower claim than first drafted. The redundant generic note was merged into the pre-existing one (its "recon checklist" and "don't confuse with parameter pollution/request smuggling" sections were real additions, worth keeping) and then deleted, rather than leaving two near-duplicate notes in the vault.
- **Process note for future sessions**: before adding a new vault note, do a full recursive listing of `vault/` (e.g. `Glob "*" vault/` — non-recursive glob patterns like `vault/**/*.md` can silently return nothing if the tool's default path context has drifted, as happened here), not just a couple of spot-checked files — this vault is no longer small enough to eyeball.

### Session update (2026-08-05) — first subprocess/shell-surface tool: radare2_analyze bridges the WSL toolchain in
- **The WSL toolchain (`scripts/install_ctf_tools.sh`, 56/58 tools verified) was sitting completely unused by the actual agent** — every existing tool is deliberately pure-Python with no subprocess/shell surface (a real, documented scope decision, see §"Explicitly scoped out for this event" above). `agent/tools/radare2_analyze.py` is the first deliberate exception to that, added because Reverse Engineering challenges genuinely need a real disassembler and reasoning about disassembly from memory isn't a substitute.
- **Scope, chosen deliberately narrow**: read-only static analysis only — `info` (rabin2 -I: arch/bits/canary/NX/PIE), `strings` (rabin2 -zz), `symbols` (rabin2 -s), `disasm` (r2 `aaa; pdf @ <symbol>`, defaults to `main`), `gadgets` (ROPgadget, from the venv). No write/patch radare2 commands, and the target binary is never executed, only inspected. `pwntools` and `angr` were deliberately **not** wrapped: pwntools is an open-ended scripting library (safely wrapping it means either letting the model execute arbitrary Python in the WSL venv, or re-building a chunk of what `tcp_session.py` already does safely in pure Python — a separate, larger design task), and `angr` is slow enough to fight the existing per-tool timeout model every other tool follows.
- **Safety**: every mode is a fixed argv template, invoked via `subprocess.run(["wsl.exe", "-e", *argv], ...)` — never `shell=True`, never a command string built from model-supplied values. `gadgets` is the one partial exception (needs `bash -lc` for `~` expansion to reach the venv), but the `-c` string passed to bash is a fixed literal with zero model input in it; the actual file path is passed as a separate argv element (`$1`), never concatenated into that string. `disasm`'s `symbol` arg (the only genuinely model-supplied value that reaches a command string, since r2's own `-c` flag takes a semicolon-separated command line, not a single argv token) is validated against `^[A-Za-z0-9_.@:]{1,128}$` before use — rejects anything shaped like `;`/backtick/quote injection. Hard-capped at a 20s timeout and 8 KB output (mirrors `fetch_url`'s cap), input capped at 20 MB, temp file always cleaned up in a `finally` block (confirmed no leftover files after repeated runs). Output wrapped in `<untrusted_data>` tags like every other tool that returns challenge-supplied content — a CTF binary's embedded strings are exactly the kind of thing those tags exist for.
- **Verified for real, not mocked**: `evals/test_tools_smoke.py` now pulls a real ELF (`/bin/true`) live through `wsl.exe cat`, base64-encodes it, and runs it through all the failure/success paths (`info` returns real ELF metadata, `strings`/`gadgets`/`disasm` all confirmed manually against real output including a real disassembled `main` and real ROP gadgets, unknown-mode and invalid-base64 both degrade to a clean string not a crash, a `symbol` injection attempt like `"main; !rm -rf /"` is rejected before it ever reaches a command string). Full existing smoke suite still passes.
- **Built in an isolated worktree** (`agent/wsl-radare2-tool` branch) specifically to avoid touching `agent/graph.py` while another session was actively using it for live picoCTF training — same reasoning as the dashboard redesign's worktree.

### Session update (2026-08-05, later) — SSH tool added, radare2_analyze's first real target solved, then the agent solved it itself
- **User's actual question that triggered this**: "how do I want my agent to solve the challenge, do I need to give it access to SSH?" — not full shell access. Answer implemented: narrow scope, matching the design bar every other live-target tool in this repo already sets. `agent/tools/ssh_session.py` adds exactly two capabilities, not a general-purpose remote shell: `ssh_analyze_binary` (SFTP-fetch a named remote file and run `radare2_analyze`'s static analysis on it server-side, in one call) and `ssh_run` (run exactly one remote command with at most one line of piped stdin, capture output — no chaining further commands within a call). Both gated through the exact same host-allowlist + optional HITL approval mechanism (`_NETWORK_TOOL_HOST_ARG`, `act()`) every other live-target tool already uses; no new safety mechanism needed, the existing one already covers this.
- **`radare2_analyze.py` refactored**, not duplicated: extracted `analyze_binary_bytes(content, mode, symbol)` as a shared helper so both the base64-inline path (`radare2_analyze`) and the SFTP-fetched path (`ssh_analyze_binary`) run through identical, already-tested logic. `ssh_analyze_binary` exists specifically so a binary's raw bytes never have to survive a model-generated tool call as a giant base64 string — the exact fragility already flagged elsewhere in this file (`gemini-3.5-flash-lite` mis-transcribing a large base64 blob) — same reasoning `fetch_and_decode_cipher` already applies to ciphertext, just for a binary payload instead of a text one.
- **New dependency**: `paramiko`, added to `requirements.txt`. Clean install on Windows (prebuilt wheels for `bcrypt`/`pynacl`, no compile step) — a real, deliberate exception to every other tool in this repo staying dependency-free, justified because SSH is genuinely a first-class capability now, not a one-off script.
- **First real target: picoCTF's "Bypass Me" (Reverse Engineering, Medium), a password-protected setuid ELF64 delivered over SSH.** Verified in two passes:
  1. Tools called directly (not through the agent loop) first, to validate the mechanics against a real target: `ssh_analyze_binary` correctly disassembled `decode_password`, revealing an 11-byte XOR-`0xAA` constant; decoded programmatically (never by hand, per this project's own anti-fabrication rule); `ssh_run` then actually ran the binary and submitted the decoded password (`SuperSecure`) over an interactive SSH channel, capturing the real flag.
  2. **Then re-run through the actual `agent.graph` loop, fresh instance, the same prompt a human gets off the platform — solved end-to-end, no hand-holding.** Triaged to `reverse` correctly. Used `ssh_analyze_binary` three times to understand `decode_password`/`sanitize`/`main`. Computed the XOR decode itself via a `python3 -c` one-liner run remotely over `ssh_run` — the anti-fabrication system-prompt rule held under a real, harder case than the one it was originally written for (Bookmarklet's simpler subtract cipher). Took genuine wrong turns getting interactive input right (`pexpect` unavailable on the remote box, a self-inflicted `subproceess` typo in its own generated Python) before succeeding via `ssh_run`'s `stdin_text` — recovered on its own, no intervention. Reached the real flag, `picoCTF{REDACTED}`, in 12 of its 15-step budget — the most steps any solved challenge has needed so far, worth knowing as a data point if a future RE/pwn run runs close to `MAX_STEPS`.
...
- **Trigger**: picoCTF's "Even RSA Can Be Broken" (Cryptography) — a netcat service printing `N`,
  ...
  on it through the dashboard; the flag (`picoCTF{REDACTED}`) was taken by hand first.
...
│       │                                (info/strings/symbols/disasm/gadgets) via radare2/
│       │                                rabin2/ROPgadget, bridged through WSL; the one tool
│       │                                with a subprocess/shell surface, allowlisted + argv-only
│       └── ssh_session.py           ← done, verified — ssh_analyze_binary (SFTP-fetch + analyze
│                                        server-side, one call) and ssh_run (one remote command +
│                                        optional piped stdin); not a general-purpose shell,
│                                        host-allowlist/HITL gated like every other live-target tool
