...
**Status update — the agent core is well past a blank scaffold now.** `agent/graph.py` runs a full **triage → think → act → observe → trim_context** loop: it classifies each challenge into a category first (web/crypto/pwn/reverse/forensics/malware/osint/misc/ai-ml/blue-team), then reasons with tools tailored to that category, and caps its own context so it doesn't grow unbounded on long runs. Tools so far: `find_flag_pattern`, `identify_and_decode`, `search_vault` (the team's own notes — this is where your `vault/` category notes and Farhan's real content matter), `search_skills` (a much bigger addition: **14 vetted third-party technique-reference packs are already installed** under `.agents/skills/` — 11 offensive, covering web/crypto/pwn/reverse/forensics/malware/osint/misc/ai-ml, plus 3 defensive/blue-team ones (incident-response reports, SIEM detection rules, SOAR playbooks), all read directly and vetted, logged in `SKILLS_VETTING.md`), and — new — three **live-network tools**: `fetch_url` (HTTP GET/POST against a real URL), `tcp_open`/`tcp_send`/`tcp_close` (a multi-turn `nc`-style TCP session — connect, log in, run a command, all across separate tool calls), and `port_scan` (sweeps a port list and grabs any banner a service volunteers, e.g. SSH's version string). The default model is `gemini-3.5-flash-lite` (get a free key at aistudio.google.com — this is the specific one that works, not just any Gemini model, see `CLAUDE.md`'s Current Status section for why). There's a one-command demo for a static challenge (`python -m demo.run_demo`) and one for a live-network challenge (`python -m demo.run_demo_network`) — good places to see exactly what the agent's input/output shape looks like for each style.

**Important shift for Rashid specifically:** `identify_and_decode`, `find_flag_pattern`, `fetch_url`, `tcp_open`/`tcp_send`/`tcp_close`, and `port_scan` are **all already built and tested** — don't re-build any of them. Web/network challenges are now a confirmed real category (the organizer's own demo showed a challenge handed out as a bare IP), so if Web is one of your picks, the connection tooling is done. Your job now is: pick the final 2–3 categories, build `extract_metadata` only if Forensics gets picked (still the one gap), and — the actually-most-important part — validate the agent solves real challenges in whatever you pick, not just that the tools work in isolation. See "Step 0" and the eval section below.

**Important shift for Hasif:** the API bridge is now built — `agent/api.py`. Run it with `uvicorn agent.api:app --reload --port 8000` (add `fastapi`/`uvicorn` — already in `requirements.txt`). Two endpoints: `POST /solve` (send `{"prompt": "...", "target": "10.0.0.5:1337"}` — `target` is optional, drop it entirely for a plain-text challenge — get back `{category, steps, flag, final_answer, tool_calls}` once the whole run finishes) and `POST /solve/stream` (same thing, but Server-Sent Events — one event per graph node, exactly the live step-by-step trace your dashboard is meant to show). The optional `target` field is there specifically so your UI can offer a separate host:port input box instead of making users type it into the prompt text — use it or not, whichever fits your design better. Point `app/api/agent/route.ts` at whichever endpoint fits your UI better — `/solve/stream` if you want the trace to appear incrementally, `/solve` if a single "done" state is simpler to wire up first. CORS is already open for `localhost:3000`.

**A note on how this actually gets used on the day:** the agent is no longer purely a "paste it in" copilot — it can now genuinely reach out to a live target on its own (`fetch_url`/`tcp_session`/`port_scan`), which matters because the organizer's demo showed a challenge handed out as just an IP. It's still not unattended, though: every network call is checked against a host allowlist derived from the prompt (it refuses to touch a host you didn't mention), has a hard timeout, and the whole loop caps out at 15 steps or 3 identical repeated tool calls. Several organizer questions are still open (scoring format, exact autonomy expectations, environment provisioning) — see `NEXT_STEPS.md` Phase 2.
...
### Your actual task — build whatever connection tool is still missing
`identify_and_decode`, `find_flag_pattern`, `fetch_url` (HTTP GET/POST against a real target), and `tcp_open`/`tcp_send`/`tcp_close` (multi-turn TCP/`nc`-style interaction) are **all already built and tested** (`agent/tools/`) — don't redo any of these. What's left, for whichever categories you picked:
